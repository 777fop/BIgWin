...
const Dashboard: React.FC<DashboardProps> = ({ user, onUserUpdate, onLogout }) => {
  ...
  const handleClaim = async () => {
    try {
      ...
      const updatedUser = {
        ...user,
        balance: result.newBalance || (user.balance + currentPlan.dailyClaim),
        totalEarned: result.totalEarned || (user.totalEarned + currentPlan.dailyClaim),
  ...
  const handleUpgrade = async (plan: Plan) => {
    try {
      await ApiService.requestUpgrade(plan.id);
      alert('Upgrade request submitted successfully! Please wait for admin approval.');
